...
struct ShelfSampleList: View {
    ...
    var body: some View {
        NavigationView {
            List {
                Section(
                    footer: Text("add all samples or select \("Shelf")")
                ) {
                    Button("Add all Sample \("Shelfs")") { showAction = true }
                }
                ...
                Section(
                    ...
                    ForEach(sampleStoreWindows[0].shelfs, id: \.self) { shelf in
                        ShelfSampleRow(storeWindow: $storeWindow,
                                       shelf: shelf)
                            .contentShape(Rectangle())
                            .onTapGesture { addSelected(shelf: shelf) }
                    }
                }
            }
