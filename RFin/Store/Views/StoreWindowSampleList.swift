...
struct StoreWindowSampleList: View {
    ...
    var body: some View {
        NavigationView {
            List {
                Section(
                    footer: Text("add all samples or select \("StoreWindow")")
                ) {
                    Button("Add all Sample \("StoreWindows")") { showAction = true }
                }
                
                Section(
                    header: Text("Sample StoreWindows"),
                    ...
                    ForEach(sampleStoreWindows, id: \.self) { storeWindow in
                        StoreWindowSampleRow(storeWindow: storeWindow)
                            ...
                            .onTapGesture { addSelected(storeWindow: storeWindow) }
                    }
                }
            }
